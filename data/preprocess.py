...
train_adjacency_folder_str = 'train_adjacency_matrices_perday_coldstart'
...
rating_threshold = 5
num_days_for_mat = 2000000000000000
...
cold_start_setting = True

def load_amazoninstantvideo(rating_file_path):
    iStream_ratings = gzip.open(rating_file_path,'r')
    ...
    for eachline in iStream_ratings:
        ...
        if i % 20000 == 0:
            print(i)
        allparts = eval(eachline)
        user_id = allparts['reviewerID']
        item_id = allparts['asin']
        rating = float(allparts['overall'])
        if binary_setting:
        ...
        timestamp = int(allparts['unixReviewTime'])
...
def create_adjacency_mat(user_id_list, user_id_list_lesspos, item_id_list, first_interact_date, last_interact_date, all_ratings):
    ...
    recurrent_rating_fullmat = sparse.dok_matrix((len(user_id_list)+len(user_id_list_lesspos), len(item_id_list)), dtype=np.float32)
    ...
    for user_id, user_ratings in all_ratings.items():
        ...
        if user_id in user_id_list:
            ...
            if cold_start_setting == True:
                sorted_ratings = sorted_ratings[0:3]
            if is_ratio_split:
            ...
            if cold_start_setting == True and len(user_training_data) == 2:
                user_training_data = user_training_data[0:1]
                test_set[user_id] = user_training_data[1:2]
            for eachrating in user_training_data:
                item_id = eachrating[0][0]
                rating = eachrating[0][1]
                happen_date = eachrating[1]
    ...
    while move_date <= last_interact_date:
        ...
        if not is_accumulated:
            recurrent_rating_fullmat = sparse.dok_matrix((len(user_id_list) + len(user_id_list_lesspos), len(item_id_list)),
                                            dtype=np.float32)
            previous_rating_fullmat = copy.deepcopy(recurrent_rating_fullmat)
            num_ratings_in_train = 0
        ...
        if move_date_str not in training_set:
            if diff_days_from_first % days_per_mat == 0 or (move_date == last_interact_date and diff_days_from_first % days_per_mat != 0):
                if np.sum(np.absolute(np.subtract(recurrent_rating_fullmat, previous_rating_fullmat))) > 0:
                    previous_rating_fullmat = copy.deepcopy(recurrent_rating_fullmat)
                    recurrent_rating_spmat = recurrent_rating_fullmat.tocsr()
...
if __name__ == '__main__':
    ml_rate_file = 'reviews_Digital_Music_5.json.gz'
    user_id_list, user_id_list_lesspos, item_id_list, first_interact_date, last_interact_date, num_interact_eachdate, all_ratings = load_amazoninstantvideo(ml_rate_file)
